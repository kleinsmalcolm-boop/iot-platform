...
app.get('/api/my-devices', authMiddleware, async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT d.*, (SELECT key_value FROM api_keys WHERE device_id = d.id AND active = true LIMIT 1) as api_key FROM devices d WHERE (d.org_id = $1 OR d.user_id = $2) ORDER BY d.created_at DESC',
